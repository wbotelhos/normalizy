...
RSpec.describe Normalizy::Filters::Date do
  ...
  it { expect(subject.call(Struct.new(:beginning_of_day).new(:success), adjust: :begin)).to eq :success }
  it { expect(subject.call(Struct.new(:end_of_day).new(:success), adjust: :end)).to eq :success }
